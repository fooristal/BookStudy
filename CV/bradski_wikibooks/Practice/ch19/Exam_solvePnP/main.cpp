...
void help(char ** argv)
{
    std::cout << "\nExample 19-01, using homography to get a bird's eye view."
              ...
              << "\n   but here, that file is already stored in F:/98_BookStudy/CV\bradski_wikibooks/Data/birdseye/intrinsics.xml"
              << "\nCall:"
...
{    
    ...
    if (image0.empty()) {
        std::cout << "Error: Couldn't load image " << argv[4]
                  << std::endl;
         return -1;
    }
    ...
    if (!found) {
        std::cout << "Couldn't acquire checkerboard on " << argv[4]
                  << ", only found " << corners.size()
                  << " of " << board_n << " corners\n";
        return -1;
    }
    ...
    //
    cv::cornerSubPix(
                gray_image,         // Imput image
                corners,            // Initial quesses, also output
    ...
    //
    std::cout << "\nPress 'd' for lower birdeye view, " <<
                 "and 'u' for higher (it adjusts the apparent 'Z' height),"<<
    ...
    cv::solvePnP(object_points, // 3-d points in object coordinate
                 ...
                 cv::Mat(),     // Since we corrected dirtortion in the beginning,
