...
void help(char ** argv)                 // todo rewrite this
{
    std::cout << "\n\n" <<
                 ...
                 "              reading and collecting the requestd number of views,\n" <<
                 "              and calibrating the camera\n\n" <<
                 "Call:\n" << argv[0] <<
                 " <board_width> <board_height> <number_of_boards> <if_video,_delay_between_frame_capture> <image_scaling_factor>\n\n" <<
                 ...
                 " * It displays and undistorted image\n" << std::endl;
}
                                        //
int main(int argc, char ** argv)
{
    ...
    while (image_points.size() < (size_t)n_boards) {
        ...
        if (!found) {
            std::cout << "Couldn't acquire checkerboard\n";
            return -1;
        }
    ...
    // EXAMPLE OF LOADING THESE MATRICS BACK IN:
